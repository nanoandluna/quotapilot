...
export default function Home() {
  ...
  const [importFeedback, setImportFeedback] = useState<Array<{ row: number; reason: string; fields: string[] }>>([]);
  ...
  const membersQuery = trpc.quota.members.useQuery({ workspaceId: activeWorkspaceId ?? 0 }, { enabled: Boolean(activeWorkspaceId) });
  const modelVersionsQuery = trpc.quota.modelVersions.useQuery({ workspaceId: activeWorkspaceId ?? 0 }, { enabled: Boolean(activeWorkspaceId) && activeView === "pool" });
  const utils = trpc.useUtils();
  const importUsage = trpc.quota.importUsage.useMutation({
    onSuccess: result => {
      setImportFeedback(result.errors);
      toast.success("历史消耗已导入", { description: result.rejected ? `已写入 ${result.accepted} 条；${result.rejected} 条被拒绝，请查看行级反馈。` : `已写入 ${result.accepted} 条用量事件。` });
  ...
  const createTask = trpc.quota.createTask.useMutation({
    onSuccess: result => {
      ...
      toast.success(result.reserved ? "硬额度已预留" : result.softReserved ? "P2 软预留已建立" : `路由决策：${result.admission}`, { description: result.softReserved ? "该任务保留队列意图，但不占用 P0/P1 的共享硬额度。" : decisionCopy[result.admission] });
      setTaskOpen(false);
      if (activeWorkspaceId) utils.quota.dashboard.invalidate({ workspaceId: activeWorkspaceId });
    },
  ...
  const claimTask = trpc.quota.claimTask.useMutation({
    onSuccess: result => {
      toast.success("本地执行已领取", { description: result.claimKind === "soft_upgraded" ? "P2 软预留已升级为硬预留；未调用 provider。" : result.claimKind === "p3_hard" ? "P3 已通过执行前二次准入并暂时锁定额度；未调用 provider。" : "已有硬预留已确认；未调用 provider。" });
      if (activeWorkspaceId) utils.quota.dashboard.invalidate({ workspaceId: activeWorkspaceId });
    },
    onError: error => toast.error("无法领取执行", { description: error.message }),
  });
  ...
  const inviteMember = trpc.quota.inviteMember.useMutation({
    onSuccess: result => {
      toast.success("邀请记录已建立", { description: `有效至 ${formatDate(result.expiresAt)}；请复制并安全投递下方 token。` });
  ...
  const acceptInvite = trpc.quota.acceptInvite.useMutation({
    onSuccess: result => {
      ...
      setInviteToken("");
      toast.success("工作区邀请已接受", { description: `已以 ${result.role} 身份加入工作区。` });
    },
    onError: error => toast.error("无法接受邀请", { description: error.message }),
  });
  ...
  const removeMember = trpc.quota.removeMember.useMutation({
    ...
    onError: error => toast.error("成员移除失败", { description: error.message }),
  });
  ...
  const hardReservations = activeReservations.filter(reservation => reservation.reservationKind === "hard");
  const softReservations = activeReservations.filter(reservation => reservation.reservationKind === "soft");
  ...
  const submitTask = () => {
    ...
    createTask.mutate({
      ...
      requirements: {
        ...
        ...(Number(maxContextTokens) > 0 ? { maxContextTokens: Number(maxContextTokens) } : {}),
      },
    ...
    <div className="dashboard-shell qv2-shell">
      ...
      <main className="main-canvas">
        ...
        <div className="content-frame">
          ...
          {activeView === "overview" && <>
            <section className="hero-block qv2-hero"><div className="hero-copy"><div className="eyebrow"><span className="eyebrow-rule" /> QUOTA-AWARE RESEARCH SCHEDULER <span className="sim-chip">V2 / DB-BACKED</span></div><h1>Budget is shared.<br /><span>Continuity is protected.</span></h1><p>额度不再按模型虚构余额，而是由共享预算、任务预留、动态保护仓和结果账本共同调度。</p><div className="hero-decision-line"><span className="decision-square" /> {hardReservations.length} 个硬预留正保护 P0/P1 任务{softReservations.length ? `；另有 ${softReservations.length} 个 P2 软预留` : ""} <ArrowRight size={13} /></div></div><div className="hero-aside"><div className="hero-aside-label">NEXT WINDOW RESET</div><div className="runway-number">{formatReset(fiveHourBudget?.resetAt)}</div><div className="runway-caption"><StatePill state={fiveHourBudget?.state} /><span>共享窗口可用 {formatUsd(availableUsd)}</span></div><div className="reserve-chip"><LockKeyhole size={12} /> DYNAMIC RESERVE {formatUsd(reserveUsd)}</div></div></section>
          ...
          {activeView === "pool" && <section className="view-section"><div className="view-intro"><div><div className="eyebrow"><span className="eyebrow-rule" /> MODEL CAPABILITY MATRIX</div><h1>Abilities are explicit.<br /><span>Scarcity is deliberate.</span></h1><p>模型目录来自数据库注册表；未来同步会以 provider 目录为源，而非以静态请求数假装独立余额。</p></div><button className="secondary-action" onClick={() => { dashboard.refetch(); modelVersionsQuery.refetch(); }}><RefreshCw size={15} /> Refresh registry</button></div><div className="capability-table panel"><div className="capability-head"><span>MODEL</span><span>CODE</span><span>REASON</span><span>CONTEXT</span><span>AGENT</span><span>RELIABLE</span><span>SCARCITY</span><span>CONC.</span></div>{data?.models.map(model => { const cap = model.capability as Record<string, number>; return <div className="capability-row" key={model.id}><b>{model.displayName}</b><span>{cap.code}</span><span>{cap.reasoning}</span><span>{cap.longContext}</span><span>{cap.agent}</span><span>{cap.reliability}</span><span className={Number(model.scarcityFactor) >= .75 ? "text-oxide" : "text-teal"}>{Number(model.scarcityFactor).toFixed(2)}</span><span>×{model.maxConcurrency}</span></div>; })}</div><section className="panel decision-history"><div className="panel-heading"><div><div className="section-kicker">MODEL METADATA HISTORY</div><h2>Version evidence</h2></div><span className="history-total">{modelVersionsQuery.data?.length ?? 0} records</span></div>{modelVersionsQuery.data?.length ? <div className="decision-history-list">{modelVersionsQuery.data.slice(0, 12).map(version => <div className="decision-history-row" key={version.id}><span className={`decision-pill ${version.isActive ? "decision-admit" : "decision-hold"}`}>{version.isActive ? "ACTIVE" : "ENDED"}</span><div><b>{version.displayName} · {version.pricingVersion}</b><small>{version.metadataSourceUrl || "workspace policy"} · verified {version.metadataVerifiedAt ? formatDate(version.metadataVerifiedAt) : "pending"} · effective {formatDate(version.effectiveFrom)}{version.effectiveUntil ? ` → ${formatDate(version.effectiveUntil)}` : " → current"}</small></div><span className="handoff-mark">cap {version.capabilityVersion}</span></div>)}</div> : <EmptyState icon={History} title="暂无模型版本记录" body="登录初始化后会写入当前工作区模型策略；历史版本仅用于审计，路由只使用 ACTIVE 版本。" />}</section></section>}
          ...
          {activeView === "ledger" && <section className="view-section"><div className="view-intro"><div><div className="eyebrow"><span className="eyebrow-rule" /> EXPERIMENT LEDGER</div><h1>Never lose the<br /><span>route of a result.</span></h1><p>账本区分 requested model、actual model、fallback reason、成本、quota state、run id 与 result class，保护科研结果纯度。</p></div><button className="secondary-action" onClick={() => dashboard.refetch()}><RefreshCw size={15} /> Refresh ledger</button></div><div className="panel ledger-table"><div className="ledger-head"><span>ATTEMPT</span><span>REQUESTED → ACTUAL</span><span>STATE</span><span>EST. COST</span><span>RESULT CLASS</span><span>ACTION</span></div>{data?.attempts.length ? data.attempts.map(attempt => <div className="ledger-row" key={attempt.id}><span>#{attempt.taskId}.{attempt.attemptNumber}</span><span>{attempt.requestedModelId || "—"} <ArrowRight size={12} /> {attempt.actualModelId || "—"}</span><span>{attempt.status}</span><span>{formatUsd(attempt.estimatedCostUsd, 4)}</span><span className={`result-class result-${attempt.resultClass}`}>{attempt.fallback ? attempt.fallbackReason || attempt.resultClass : attempt.resultClass}</span>{attempt.status === "queued" ? <button className="ledger-settle" disabled={!activeWorkspaceId || claimTask.isPending} onClick={() => activeWorkspaceId && claimTask.mutate({ workspaceId: activeWorkspaceId, taskId: attempt.taskId })}>Claim locally</button> : <button className="ledger-settle" disabled={!activeWorkspaceId || recordAttempt.isPending || attempt.status === "completed" || attempt.status === "failed" || attempt.status === "cancelled"} onClick={() => settleAttempt(attempt)}>{attempt.status === "running" ? "Settle" : "Closed"}</button>}</div>) : <EmptyState icon={FileDown} title="暂无实验 attempt" body="真实任务创建后，每一次调度、预留和 fallback 都会在此留下不可变证据。" />}</div><div className="ledger-note"><ShieldCheck size={17} /><span>“Claim locally” 会执行本地额度二次准入并改变队列状态，但不会调用 provider；“Settle” 仅记录本地/人工账本结算。</span></div><section className="panel decision-history"><div className="panel-heading"><div><div className="section-kicker">ROUTE DECISION HISTORY</div><h2>Why the queue moved</h2></div><span className="history-total">{data?.decisions.length ?? 0} records</span></div>{data?.decisions.length ? <div className="decision-history-list">{data.decisions.slice(0, 8).map(decision => <div className="decision-history-row" key={decision.id}><span className={`decision-pill decision-${decision.admissionDecision.toLowerCase()}`}>{decision.admissionDecision}</span><div><b>{decision.reason}</b><small>available {formatUsd(decision.availableUsd, 3)} · reserve {formatUsd(decision.dynamicReserveUsd, 3)} · estimate {formatUsd(decision.estimatedCostUsd, 3)}</small></div>{decision.actedAt ? <span className="handoff-mark">{decision.selectedModelId ? `已迁移 · ${decision.selectedModelId}` : "已处理"}</span> : decision.admissionDecision === "MIGRATE" ? <div className="decision-controls"><select value={candidateModelByDecision[decision.id] ?? ""} onChange={event => setCandidateModelByDecision(current => ({ ...current, [decision.id]: event.target.value }))}><option value="">选择迁移模型</option>{data.models.map(model => <option key={model.id} value={model.modelId}>{model.displayName}</option>)}</select><button className="decision-action" disabled={!activeWorkspaceId || !candidateModelByDecision[decision.id] || actOnRouteDecision.isPending} onClick={() => activeWorkspaceId && actOnRouteDecision.mutate({ workspaceId: activeWorkspaceId, decisionId: decision.id, action: "migrate", candidateModelId: candidateModelByDecision[decision.id] })}>确认迁移</button></div> : decision.admissionDecision === "QUEUE" ? <button className="decision-action" disabled={!activeWorkspaceId || actOnRouteDecision.isPending} onClick={() => activeWorkspaceId && actOnRouteDecision.mutate({ workspaceId: activeWorkspaceId, decisionId: decision.id, action: "queue" })}>继续排队</button> : decision.admissionDecision === "HOLD" ? <div className="decision-controls"><button className="decision-action muted-action" disabled={!activeWorkspaceId || actOnRouteDecision.isPending} onClick={() => activeWorkspaceId && actOnRouteDecision.mutate({ workspaceId: activeWorkspaceId, decisionId: decision.id, action: "hold" })}>保持暂停</button><button className="decision-action" disabled={!activeWorkspaceId || actOnRouteDecision.isPending} onClick={() => activeWorkspaceId && actOnRouteDecision.mutate({ workspaceId: activeWorkspaceId, decisionId: decision.id, action: "manual_handoff" })}>人工交接</button></div> : <span className="handoff-mark">无需操作</span>}</div>)}</div> : <EmptyState icon={Route} title="暂无 route decision" body="创建预算任务后，准入阈值、建议动作与人工接管要求会在此留存。" />}</section></section>}

          {activeView === "team" && <section className="view-section"><div className="view-intro"><div><div className="eyebrow"><span className="eyebrow-rule" /> TEAM & ACCESS</div><h1>Research continuity<br /><span>needs clear authority.</span></h1><p>owner、admin、researcher、reviewer 与 viewer 按最小权限管理预算策略、导入、任务和结果审查。</p></div><div className="flex gap-2"><button className="secondary-action compact-action" onClick={() => setAcceptInviteOpen(true)}><CheckCircle2 size={15} /> Accept invite</button><button className="primary-action compact-action" disabled={!workspaceId} onClick={() => { setCreatedInviteToken(null); setInviteOpen(true); }}><Users size={15} /> Invite member</button></div></div><div className="team-grid"><div className="panel"><div className="section-kicker">MEMBERS</div><h2>Workspace roles</h2>{membersQuery.data?.length ? <div className="member-list">{membersQuery.data.map(member => <div className="member-card" key={member.id}><span className="profile-avatar">{member.userId === user?.id ? (user?.name?.slice(0, 1).toUpperCase() || "R") : "M"}</span><div><b>{member.userId === user?.id ? user?.name || "Current researcher" : `Member #${member.userId}`}</b><small>{member.userId === user?.id ? "当前账户" : `加入于 ${formatDate(member.createdAt)}`}</small></div>{member.role === "owner" ? <span className="role-stamp">OWNER</span> : <div className="member-controls"><select value={member.role} disabled={!activeWorkspaceId || changeMemberRole.isPending} onChange={event => activeWorkspaceId && changeMemberRole.mutate({ workspaceId: activeWorkspaceId, memberUserId: member.userId, role: event.target.value as "admin" | "researcher" | "reviewer" | "viewer" })}><option value="admin">Admin</option><option value="researcher">Researcher</option><option value="reviewer">Reviewer</option><option value="viewer">Viewer</option></select><button className="member-remove" disabled={!activeWorkspaceId || removeMember.isPending || member.userId === user?.id} onClick={() => activeWorkspaceId && window.confirm("确认移除该成员的工作区访问权限？") && removeMember.mutate({ workspaceId: activeWorkspaceId, memberUserId: member.userId })}>Remove</button></div>}</div>)}</div> : <EmptyState icon={Users} title="等待账户连接" body="登录后会建立个人工作区和成员记录。" />}</div><div className="panel"><div className="section-kicker">ROLE POLICY</div><h2>Operational boundaries</h2><div className="role-list"><span><b>OWNER / ADMIN</b><small>连接、策略、成员、调度设置</small></span><span><b>RESEARCHER</b><small>导入、创建任务、申请预留</small></span><span><b>REVIEWER</b><small>审阅 ledger、确认告警</small></span><span><b>VIEWER</b><small>只读预算与结果</small></span></div></div></div></section>}
