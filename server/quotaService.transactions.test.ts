...
describe("QuotaPilot V0.2 transaction guards", () => {
  ...
  it("admits at most one concurrent initial hard reservation against the same shared budget", async () => {
    ...
    const buildTransaction = () => ({
      update: vi.fn(() => ({
        set: () => ({
          where: async () => [
            { affectedRows: capacityClaimsRemaining-- > 0 ? 1 : 0 },
          ],
        }),
      })),
    });
  ...
  it("returns the first task result for a repeated workspace idempotency key without opening another reservation transaction", async () => {
    const db = {
      select: sequenceSelect([
        [{ id: 71, status: "reserved", admissionDecision: "RESERVE" }],
        [{ reservationKind: "hard", status: "RESERVED" }],
      ]),
      transaction: vi.fn(),
    };
    doubles.getDb.mockResolvedValue(db);

    const result = await reserveTaskBudget({
      workspaceId: 7,
      userId: 11,
      title: "重复提交保护",
      priority: "P1",
  ...
  it("records a retryable failed batch when object storage rejects an import", async () => {
    ...
    doubles.getDb.mockResolvedValue(db);
    doubles.storagePut.mockRejectedValue(
  ...
  it.each([
    ...
    "settles a %s attempt by marking its hard reservation %s and writing a usage event",
    async (status, expectedReservationStatus) => {
          ...
          [
            {
              ...
              dynamicReserveUsd: "0",
              resetPolicy: "rolling",
              resetAt: new Date("2026-08-13T18:00:00.000Z"),
              providerReportedResetAt: null,
            },
          ],
          [],
          [],
      ...
      if (status === "failed") {
        expect(transactionSets.mock.calls[0]?.[0]).toMatchObject({
          failureReason: "RATE_LIMIT",
          failurePolicy: { recommendedAction: "queue", retryAfterSeconds: 15 },
        });
        expect(result).toMatchObject({
          taskStatus: "queued",
          retryScheduledAt: expect.any(Date),
        });
        expect(transactionSets.mock.calls[1]?.[0]).toMatchObject({
          status: "queued",
          completedAt: null,
        });
        ...
        expect(transactionSets.mock.calls[3]?.[0]).toMatchObject({
          connectionState: "degraded",
          circuitReason: "RATE_LIMIT",
          circuitOpenUntil: expect.any(Date),
        });
      }
      expect(transactionSets.mock.calls[1]?.[0]).toMatchObject({
        actualCostUsd: "0.100000",
        remainingBudgetUsd: "1.900000",
      });
      expect(transactionSets.mock.calls[2]?.[0]).toMatchObject({
        status: expectedReservationStatus,
      });
      expect(eventValues).toHaveBeenCalledWith(
        expect.objectContaining({
          attemptId: 200,
      ...
      expect(eventValues).toHaveBeenCalledWith(
        expect.objectContaining({
          source: "task_attempt",
          externalRef: "attempt:200:settled",
        })
      );
      expect(eventValues).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("blocks a claim when the task remaining budget cannot cover its estimated execution cost", async () => {
        ...
        [
          {
            ...
            taskBudgetUsd: "0.800000",
            cumulativeCostCapUsd: "0.800000",
            actualCostUsd: "0.000000",
            remainingBudgetUsd: "0.500000",
          },
        ],
        [
          {
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 45 })
  ...
  it("blocks a queued retry until its failure-policy cooldown has elapsed", async () => {
        ...
        [
          {
            ...
            status: "queued",
            estimatedCostUsd: "0.100000",
            taskBudgetUsd: "0.500000",
            cumulativeCostCapUsd: "0.500000",
            actualCostUsd: "0.100000",
            remainingBudgetUsd: "0.400000",
          },
        ],
        [
          {
            ...
            status: "queued",
            provider: "opencode_go",
            retryNotBefore: new Date(Date.now() + 30_000),
          },
        ],
      ]),
      update: vi.fn(),
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 46 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
    expect(transaction.update).not.toHaveBeenCalled();
  });
  ...
  it("blocks a claim while the target provider connection is circuit-open", async () => {
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 48 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
    expect(transaction.update).not.toHaveBeenCalled();
  });
  ...
  it.each([
    {
      domain: "TIMEOUT" as const,
      priority: "P2",
      taskStatus: "queued",
      retryMode: "backoff",
      executionPlan: { outputReductionRatio: 0.7 },
      routeAction: undefined,
      circuit: false,
    },
    {
      domain: "PROVIDER_ERROR" as const,
      priority: "P2",
      taskStatus: "queued",
      retryMode: "backoff",
      executionPlan: { contextReductionRatio: 1 },
      routeAction: undefined,
      circuit: true,
    },
    {
      domain: "TOOL_ERROR" as const,
      priority: "P2",
      taskStatus: "queued",
      retryMode: "backoff",
      executionPlan: { maxToolCalls: 2, maxAgentSteps: 3 },
      routeAction: undefined,
      circuit: false,
    },
    {
      domain: "QUOTA" as const,
      priority: "P0",
      taskStatus: "paused",
      retryMode: "after_remediation",
    ...
    },
    {
      ...
      retryMode: "none",
      executionPlan: undefined,
      routeAction: "manual_handoff",
      circuit: false,
    },
  ])(
    "executes the $domain failure path",
    async ({
      domain,
      priority,
      taskStatus,
      ...
      expect(result).toMatchObject({
        taskStatus,
        failurePolicy: { retryMode },
      });
      if (executionPlan) {
        expect(insertValues).toHaveBeenCalledWith(
          expect.objectContaining({
            attemptNumber: 2,
            executionPlan: expect.objectContaining(executionPlan),
          })
        );
      } else {
        expect(insertValues).toHaveBeenCalledWith(
          expect.objectContaining({ recommendedAction: routeAction })
        );
      }
      if (circuit) {
        expect(transactionSets).toHaveBeenCalledWith(
          expect.objectContaining({
            connectionState: "degraded",
            circuitReason: domain,
            circuitOpenUntil: expect.any(Date),
          })
        );
      }
        ...
        [
          {
            ...
            taskBudgetUsd: "0.500000",
            cumulativeCostCapUsd: "0.500000",
            actualCostUsd: "0.400000",
            remainingBudgetUsd: "0.100000",
            requestedModelId: "deepseek-v4-flash",
          },
        ],
        [
  ...
  it("queues one next attempt below the configured limit and rejects retries at the limit", async () => {
    ...
    await expect(
      queueTaskRetry({ workspaceId: 7, taskId: 102 })
    ).resolves.toMatchObject({ taskId: 102, attemptId: 202, attemptNumber: 2 });
    ...
    expect(attemptValues).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        taskId: 102,
        attemptId: 202,
        payload: expect.objectContaining({ source: "manual_retry" }),
      })
    );
    ...
    doubles.getDb.mockResolvedValue({
      transaction: async (
        work: (tx: typeof limitedTransaction) => Promise<unknown>
      ) => work(limitedTransaction),
    });

    await expect(
      queueTaskRetry({ workspaceId: 7, taskId: 103 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
  ...
  it("upgrades a P2 soft reservation only after an execution-time hard budget claim succeeds", async () => {
    ...
    const result = await claimTaskForLocalExecution({
      workspaceId: 7,
      taskId: 42,
    });
  ...
  it("does not start a soft-reserved task when the execution-time budget claim loses capacity", async () => {
    const conditionalReserve = vi.fn(() => ({
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 43 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
  ...
  it("blocks a claim when the provider-level concurrency budget is full", async () => {
    const transaction = {
      ...
      update: vi.fn(() => ({
        set: vi.fn(() => ({ where: async () => [{ affectedRows: 0 }] })),
      })),
      insert: vi.fn(),
    };
    doubles.getDb.mockResolvedValue({
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 50 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
    expect(transaction.update).toHaveBeenCalledTimes(1);
  ...
  it("resolves a legacy NULL modelRegistryId before enforcing provider concurrency instead of bypassing the three-tier budget", async () => {
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 52 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
  ...
  it("releases a provider slot when the model-level concurrency claim loses its race", async () => {
    ...
    const transaction = {
      ...
      update: vi.fn(() => ({
        set: vi.fn(() => ({
          where: async () => [{ affectedRows: updateResults.shift() ?? 1 }],
        })),
      })),
      insert: vi.fn(() => ({
    ...
    await expect(
      claimTaskForLocalExecution({ workspaceId: 7, taskId: 51 })
    ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
  ...
  it("admits at most one concurrent P2 soft-to-hard claim against the same remaining shared budget", async () => {
    ...
    const db = {
      transaction: vi.fn(
        (work: (tx: (typeof transactions)[number]) => Promise<unknown>) =>
  ...
  it("admits at most one concurrent P3 claim when neither task has a prior reservation", async () => {
    ...
    const transactions = [
      buildP3Transaction(70, 270),
      buildP3Transaction(71, 271),
    ];
    const db = {
      transaction: vi.fn(
        (work: (tx: (typeof transactions)[number]) => Promise<unknown>) =>
  ...
  it("shares the same execution-time capacity between a P2 soft reservation and a P3 best-effort task", async () => {
    ...
    const buildTransaction = (
      taskId: number,
      attemptId: number,
      reservationKind: "soft" | undefined
    ) => {
      const conditionalBudgetResult = capacityClaimsRemaining-- > 0 ? 1 : 0;
      const updateResults =
        conditionalBudgetResult === 1
          ? reservationKind
            ? [1, 1, 1, 1]
            : [1, 1, 1]
          : [0];
    ...
    const transactions = [
      buildTransaction(80, 280, "soft"),
      buildTransaction(81, 281, undefined),
    ];
    const db = {
      transaction: vi.fn(
        (work: (tx: (typeof transactions)[number]) => Promise<unknown>) =>
