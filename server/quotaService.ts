...
export function getFailurePolicy(
  ...
  > = {
    QUOTA: {
      recommendedAction: strictResearch ? "hold" : "migrate",
      retryAfterSeconds: null,
      retryMode: "after_remediation",
      circuitScope: "provider_window",
      degradationSteps: [
        "保留正式任务预算",
        "等待共享窗口重置或选择已验证候选模型",
      ],
      requiresHumanHandoff: strictResearch,
    },
    RATE_LIMIT: {
      recommendedAction: "queue",
      retryAfterSeconds: strictResearch ? 15 : 30,
      retryMode: "backoff",
      circuitScope: "provider",
      degradationSteps: ["指数退避", "降低并发", "保留请求语义"],
    ...
    TIMEOUT: {
      ...
      retryMode: "backoff",
      circuitScope: "model",
      degradationSteps: ["缩小输出上限", "拆分任务", "降低 Agent 步数"],
      requiresHumanHandoff: false,
    },
    PROVIDER_ERROR: {
      recommendedAction: "queue",
    ...
    MODEL_UNAVAILABLE: {
      recommendedAction: strictResearch ? "manual_handoff" : "migrate",
      retryAfterSeconds: null,
      retryMode: "after_remediation",
      circuitScope: "model",
      degradationSteps: [
    ...
    CONTEXT_OVERFLOW: {
      ...
      retryMode: "backoff",
      circuitScope: "task",
      degradationSteps: ["压缩上下文", "分块输入", "减少工具输出"],
      requiresHumanHandoff: false,
    },
    TOOL_ERROR: {
      recommendedAction: "queue",
      ...
      retryMode: "backoff",
      circuitScope: "tool",
      degradationSteps: ["缩小工具调用", "重试幂等步骤", "隔离失败工具"],
      requiresHumanHandoff: false,
    },
    UNKNOWN: {
      ...
      retryAfterSeconds: null,
      retryMode: "none",
      circuitScope: "unknown",
      degradationSteps: ["保留失败证据", "人工复核后再重试"],
...
export function getFailureExecutionPlan(
  domain: FailureDomain
): FailureExecutionPlan {
  const baseline: FailureExecutionPlan = {
  ...
  const plans: Partial<Record<FailureDomain, FailureExecutionPlan>> = {
    ...
    CONTEXT_OVERFLOW: {
      ...baseline,
      contextReductionRatio: 0.7,
      outputReductionRatio: 0.8,
      maxToolCalls: 3,
      maxAgentSteps: 3,
      chunkInput: true,
      splitTask: true,
...
const OPEN_CODE_MODELS: ModelSeed[] = [
  ...
  },
  {
    ...
    scarcity: "0.350",
    concurrency: 4,
    capability: capability({ code: 7, reasoning: 7, speed: 8, reliability: 7 }),
  },
];
...
export function needsNewModelVersion(
  current: {
  ...
  return (
    current.inputPerMillionUsd !== desired.input ||
...
async function syncWorkspacePolicyModelInTransaction(
  db: any,
  model: ModelSeed
) {
  ...
  return {
    action: activeVersion ? ("replaced" as const) : ("inserted" as const),
    previousVersionId: activeVersion?.id,
  };
}
...
export async function syncWorkspacePolicyModel(db: any, model: ModelSeed) {
  return db.transaction((tx: any) =>
    syncWorkspacePolicyModelInTransaction(tx, model)
  );
}
...
function taskCostCapUsd(task: {
  cumulativeCostCapUsd: string | number | null;
  taskBudgetUsd: string | number | null;
  ...
  return explicitCap > 0
    ? explicitCap
    : taskBudget > 0
      ? taskBudget
      : asNumber(task.estimatedCostUsd);
}

export function getTaskBudgetAdmission(input: {
  ...
  if (
    ...
    !Number.isFinite(input.taskBudgetUsd) ||
    input.taskBudgetUsd <= 0
  ) {
    return {
      admitted: false,
      reason: "任务预计成本必须为非负数，任务成本上限必须为正数。",
    } as const;
  }
...
export function getTaskRetryAdmission(input: {
  attemptCount: number;
  maxAttempts: number;
}) {
...
export function resolveBudgetResetAt(input: {
  ...
  if (
    input.policy === "provider_reported" &&
    input.providerReportedResetAt &&
    input.providerReportedResetAt > now
  )
    return input.providerReportedResetAt;
...
export function calculateBudgetState(input: {
  ...
  const availableUsd = Math.max(
    0,
    input.limitUsd - input.consumedUsd - input.reservedUsd
  );
  const worstBurn = Math.max(0, ...input.burnRates);
  const forecastExhaustionAt =
    worstBurn > 0
      ? new Date(now.getTime() + (availableUsd / worstBurn) * 3_600_000)
      : null;
  const spendableBeforeReserve = Math.max(
    0,
    availableUsd - input.dynamicReserveUsd
  );
  const ratio = input.limitUsd > 0 ? availableUsd / input.limitUsd : 0;

  if (availableUsd <= 0)
    return { availableUsd, forecastExhaustionAt, state: "RED" };
  ...
  if (spendableBeforeReserve <= 0 || ratio <= 0.2)
    return { availableUsd, forecastExhaustionAt, state: "ORANGE" };
  if (ratio <= 0.4)
    return { availableUsd, forecastExhaustionAt, state: "YELLOW" };
...
export function getAdmissionDecision(input: {
  ...
  if (
    input.availableUsd < input.estimatedCostUsd ||
    input.budgetState === "RED"
  )
    return "HOLD";
  ...
  if (
    input.budgetState === "DRAIN_PROTECTION" ||
    spendableAboveReserve < input.estimatedCostUsd
  ) {
    return input.routeMode === "emergency" && input.priority === "P3"
      ? "MIGRATE"
      : "QUEUE";
  }
  if (
    input.budgetState === "ORANGE" &&
    (input.priority === "P2" || input.priority === "P3")
  )
    return "MIGRATE";
  return "ADMIT";
}

export function getReservationKind(
  priority: "P0" | "P1" | "P2" | "P3"
): "hard" | "soft" | undefined {
...
export async function claimInitialHardReservation(
  tx: any,
  providerBudgetId: number,
  amountUsd: number
) {
  const conditionalReserve = await tx
    .update(providerBudgets)
    .set({
...
export function scoreCandidateModels(
  requirements: TaskRequirements,
  models: RoutingModel[],
  options?: { priority?: "P0" | "P1" | "P2" | "P3" }
) {
  const requiredCapabilities = (
    Object.entries(requirements) as Array<[keyof TaskRequirements, unknown]>
  ).filter(
    ([key, value]) =>
      key in capability({}) && typeof value === "number" && value > 0
  ) as Array<[keyof CapabilityMatrix, number]>;
  const p0MinimumCapability = requiredCapabilities.length
    ? Math.max(...requiredCapabilities.map(([, required]) => required))
    : 7;
  return models
    .filter(model => model.maxConcurrency > 0)
    .filter(
      model => !requirements.requiresVision || model.capability.vision > 0
    )
    .filter(
      model => !requirements.requiresToolUse || model.capability.toolUse > 0
    )
    .filter(
      model =>
        !requirements.maxContextTokens ||
        (model.maxContextTokens ?? 0) >= requirements.maxContextTokens
    )
    .filter(model =>
      requiredCapabilities.every(
        ([key, required]) => model.capability[key] >= required
      )
    )
    .filter(
      model =>
        options?.priority !== "P0" ||
        (model.capability.reliability >= 8 &&
          model.capability.reasoning >= Math.min(8, p0MinimumCapability))
    )
    .map(model => {
      const capabilityFit = requiredCapabilities.length
        ? requiredCapabilities.reduce(
            (total, [key, required]) =>
              total +
              Math.min(1, model.capability[key] / Math.max(1, required)),
            0
          ) / requiredCapabilities.length
        : 0.6;
      const availability = Math.min(
        1,
        model.maxConcurrency / Math.max(1, model.activeConcurrency ?? 1)
      );
      const continuity = Math.max(0, 1 - model.scarcityFactor);
      const quality =
        capabilityFit * 0.5 +
        (model.capability.reliability / 10) * 0.3 +
        availability * 0.1 +
        continuity * 0.1;
      ...
      const p0Reason =
        options?.priority === "P0"
          ? "P0 已通过可靠性 ≥8 与推理能力门槛；后续必须通过共享额度原子硬预留。"
          : "";
      return {
        ...model,
        score: Number(score.toFixed(3)),
        reason: `能力满足硬约束；可靠性 ${model.capability.reliability}/10，可用并发 ${model.maxConcurrency}，稀缺性 ${model.scarcityFactor.toFixed(2)}。${p0Reason}`,
      };
    })
...
export function resolveTaskRouting(
  ...
  if (input.routeMode === "strict") {
    if (!input.requestedModelId) {
      return {
        ...
        reason: "严格模式必须指定 requested model；系统不会自行选择替代模型。",
      };
    }
    if (!requestedCandidate) {
      return {
        candidates,
        recommendedModelId: input.requestedModelId,
        blockedByCapability: true,
        reason:
          "指定模型未激活、并发不可用或不满足任务能力硬约束；严格模式禁止自动替换。",
      };
    }
    return {
      candidates: [requestedCandidate],
      recommendedModelId: requestedCandidate.modelId,
      blockedByCapability: false,
      reason: "严格模式保留能力合格的指定模型。",
    };
  }
  ...
  if (input.routeMode === "balanced" && requestedCandidate) {
    return {
      ...
      blockedByCapability: false,
      reason:
        "平衡模式优先采用能力合格的指定模型；若后续配额或连接校验失败，可生成替代 Route Plan。",
    };
  }
  ...
  return {
    ...
    reason:
      candidates.length === 0
        ? "没有模型满足任务能力硬约束。"
        : input.routeMode === "emergency"
          ? "应急模式按能力、可靠性、成本与稀缺性选择当前最优候选。"
          : "平衡模式未找到可用指定模型，改为选择当前最优候选。",
  };
}
...
export function buildUnifiedRoutePlan(input: {
  requirements: TaskRequirements;
  models: Array<
    RoutingModel & { provider?: "opencode_go" | "openai_api" | "local" }
  >;
  priority?: "P0" | "P1" | "P2" | "P3";
  routeMode: RouteMode;
  requestedModelId?: string;
  ...
  const base = resolveTaskRouting(input.requirements, input.models, {
    routeMode: input.routeMode,
    requestedModelId: input.requestedModelId,
    priority: input.priority,
  });
  const contextByProvider = new Map(
    input.providerContexts.map(context => [context.provider, context])
  );
  const scoredByModelId = new Map(
    base.candidates.map(candidate => [candidate.modelId, candidate])
  );
  const candidates = base.candidates.map(candidate => {
    ...
    const hasBudget =
      typeof context?.availableUsd === "number" &&
      context.availableUsd >= input.estimatedCostUsd;
    const hasP0ReservableBudget =
      input.priority !== "P0" ||
      (hasBudget &&
        (context?.availableUsd ?? 0) - (context?.dynamicReserveUsd ?? 0) >=
          input.estimatedCostUsd);
    const reasons = [candidate.reason];
    if (!context) reasons.push("未发现 provider 五小时预算上下文。");
    else if (!hasBudget)
      reasons.push(
        `provider 当前可用额度不足：${(context.availableUsd ?? 0).toFixed(4)} USD。`
      );
    else if (!hasP0ReservableBudget)
      reasons.push(
        `P0 无法在动态保护仓 ${(context?.dynamicReserveUsd ?? 0).toFixed(4)} USD 之上完成硬预留。`
      );
    if (context && context.secretState !== "configured")
      reasons.push("provider 凭据尚未配置；该计划只能人工执行或导入结算。");
    if (context && context.connectionState !== "connected")
      reasons.push(
        `provider 连接状态为 ${context.connectionState ?? "unknown"}。`
      );
    return {
      candidate,
      provider,
      context,
      hasBudget,
      hasP0ReservableBudget,
      reasons,
    };
  });
  const eligible = candidates
    .filter(candidate => candidate.hasBudget && candidate.hasP0ReservableBudget)
    .sort((left, right) => {
      const leftPressure =
        input.estimatedCostUsd /
        Math.max(
          left.context?.availableUsd ?? input.estimatedCostUsd,
          input.estimatedCostUsd
        );
      const rightPressure =
        input.estimatedCostUsd /
        Math.max(
          right.context?.availableUsd ?? input.estimatedCostUsd,
          input.estimatedCostUsd
        );
      return (
        right.candidate.score -
        rightPressure * 8 -
        (left.candidate.score - leftPressure * 8)
      );
    });
  const requested = input.requestedModelId
    ? candidates.find(
        candidate => candidate.candidate.modelId === input.requestedModelId
      )
    : undefined;
  ...
  if (input.routeMode === "strict") {
    selected =
      requested?.hasBudget && requested.hasP0ReservableBudget
        ? requested
        : undefined;
    if (!requested) blockedByCapability = true;
    if (requested && (!requested.hasBudget || !requested.hasP0ReservableBudget))
      blockedByBudget = true;
    if (blockedByBudget)
      reason =
        "严格模式指定模型满足能力要求，但其 provider 额度不足；禁止自动换模。";
  } else if (
    input.routeMode === "balanced" &&
    requested?.hasBudget &&
    requested.hasP0ReservableBudget
  ) {
    selected = requested;
    ...
    blockedByBudget = true;
    reason =
      "存在能力合格模型，但没有任何候选 provider 具备覆盖本任务的共享可用额度。";
  }
  ...
  const routePlan: RoutePlanSnapshot = {
    ...
    candidates: input.models.map(model => {
      ...
      const candidate = candidates.find(
        item => item.candidate.modelId === model.modelId
      );
...
function describeRouteDecision(
  decision: "ADMIT" | "RESERVE" | "MIGRATE" | "QUEUE" | "HOLD"
) {
  if (decision === "RESERVE")
    return {
      reason: "P0/P1 任务通过共享预算检查，需先锁定可预估成本。",
  ...
  if (decision === "MIGRATE")
    return {
      reason:
        "当前窗口接近动态保护仓；非关键任务应迁移到能力合格、成本更低的模型。",
  ...
  if (decision === "QUEUE")
    return {
      reason:
        "动态保护仓或 DRAIN_PROTECTION 正在保护 P0/P1 连续性，任务需等待预算恢复。",
      action: "queue" as const,
      human: false,
    };
  if (decision === "HOLD")
    return {
      reason: "共享窗口余额不足以覆盖任务预估成本，不能静默降级。",
      action: "manual_handoff" as const,
      human: true,
    };
  return {
    reason: "能力、成本和共享预算均满足当前任务准入条件。",
    action: "run" as const,
    human: false,
  };
}
...
export function parseUsageImportDetailed(
  content: string,
  format: "csv" | "json"
): { events: UsageImportRecord[]; errors: UsageImportRowError[] } {
  const rawRows: Record<string, unknown>[] =
    format === "json" ? parseJsonRows(content) : parseCsvRows(content);
  ...
  rawRows.forEach((row, index) => {
    const value = (keys: string[]) => {
      const key = keys.find(
        candidate => row[candidate] !== undefined && row[candidate] !== ""
      );
      return key ? row[key] : undefined;
    };
    const provider = String(
      value(["provider", "provider_id"]) ?? "opencode_go"
    );
    const modelId = String(
      value(["model_id", "model", "modelId"]) ?? ""
    ).trim();
    const occurredRaw = value([
      "occurred_at",
      "timestamp",
      "occurredAt",
      "time",
    ...
    const parsedActual =
      actualCost === undefined ? undefined : Number(actualCost);
    const parsedEstimate =
      estimatedCost === undefined ? 0 : Number(estimatedCost);
    if (
      ...
      errors.push({
        row: index + 1,
        reason: "成本字段必须为数字。",
        fields: ["actual_cost_usd", "estimated_cost_usd"],
      });
      return;
    }
    ...
    events.push({
      ...
      tokens: {
        ...
        cacheReadTokens: numeric(
          value(["cache_read_tokens", "cacheReadTokens"])
        ),
      ...
      occurredAt,
      externalRef:
        String(value(["external_ref", "externalRef", "id"]) ?? "").trim() ||
        undefined,
    });
  });
...
export function parseUsageImport(
  content: string,
  format: "csv" | "json"
): UsageImportRecord[] {
...
export async function ensurePersonalWorkspace(user: {
  ...
  await db
    .insert(workspaces)
    .values({
      name: `${user.name || "研究"}的 QuotaPilot`,
      slug,
      createdByUserId: user.id,
    })
    .onDuplicateKeyUpdate({ set: { updatedAt: new Date() } });
  const workspace = (
    await db.select().from(workspaces).where(eq(workspaces.slug, slug)).limit(1)
  )[0];
  if (!workspace)
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
...
async function seedWorkspaceDefaults(workspaceId: number) {
  ...
  const connectionSeeds = [
    ...
    {
      provider: "chatgpt_plus_manual" as const,
      displayName: "ChatGPT Plus · 人工救援",
      syncMode: "manual" as const,
      secretState: "not_configured" as const,
    },
  ];
  ...
  for (const seed of connectionSeeds) {
    await db
      ...
      .values({
        ...
        connectionState:
          seed.secretState === "configured"
            ? "connected"
            : "pending_configuration",
      })
      .onDuplicateKeyUpdate({
        set: { secretState: seed.secretState, updatedAt: new Date() },
      });
  }
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(providerConnections.provider, "opencode_go")
        )
      )
      .limit(1)
  )[0];
  if (goConnection) {
    ...
    for (const item of windows) {
      await db
        .insert(providerBudgets)
        .values({ workspaceId, providerConnectionId: goConnection.id, ...item })
        .onDuplicateKeyUpdate({
          set: {
...
export async function refreshWorkspaceBudgets(workspaceId: number) {
  ...
  for (const budget of budgets) {
    ...
    const events = await db
      ...
      .where(
        and(
          ...
          eq(usageEvents.providerConnectionId, budget.providerConnectionId),
          gte(usageEvents.occurredAt, start)
        )
      );
    const consumed = events.reduce(
      (sum, event) =>
        sum + asNumber(event.actualCostUsd ?? event.estimatedCostUsd),
      0
    );
    const activeReservations = await db
      ...
      .where(
        and(
          eq(budgetReservations.providerBudgetId, budget.id),
          eq(budgetReservations.status, "RESERVED"),
          eq(budgetReservations.reservationKind, "hard")
        )
      );
    const reserved = activeReservations.reduce(
      (sum, reservation) => sum + asNumber(reservation.amountUsd),
      0
    );
    const p0p1Tasks = await db
      .select()
      .from(researchTasks)
      .where(
        and(
          eq(researchTasks.workspaceId, workspaceId),
          lte(researchTasks.priority, "P1")
        )
      );
    const commitment = p0p1Tasks
      .filter(task => ["queued", "reserved", "running"].includes(task.status))
      .reduce((sum, task) => sum + asNumber(task.estimatedCostUsd), 0);
    ...
    const phaseFloor =
      asNumber(budget.limitUsd) * PHASE_RESERVE_RATIO[workspace.researchPhase];
    const burnRiskBuffer = Math.min(
      asNumber(budget.limitUsd) * 0.15,
      Math.max(burn15m, burn1h, burn5h) * 0.5
    );
    const dynamicReserve = Math.max(phaseFloor, commitment) + burnRiskBuffer;
    const state = calculateBudgetState({
      limitUsd: asNumber(budget.limitUsd),
      consumedUsd: consumed,
      reservedUsd: reserved,
      dynamicReserveUsd: dynamicReserve,
      burnRates: [burn15m, burn1h, burn5h],
      resetAt,
      now,
    });
    await db
    ...
    if (state.state !== "GREEN") {
      await createUnacknowledgedAlert({
        ...
        providerBudgetId: budget.id,
        severity:
          state.state === "RED" || state.state === "DRAIN_PROTECTION"
            ? "critical"
            : "warning",
        kind: "budget_state",
    ...
    if (
      state.forecastExhaustionAt &&
      state.forecastExhaustionAt.getTime() < resetAt.getTime()
    ) {
      await createUnacknowledgedAlert({
        workspaceId,
        providerBudgetId: budget.id,
  ...
  if (pausedTask)
    await createUnacknowledgedAlert({
      ...
      title: "任务队列存在暂停项",
      message:
        "至少一个任务无法通过共享预算准入检查；请补充额度、等待重置或手动调整任务预算。",
    });
...
function sumCostSince(
  events: Array<typeof usageEvents.$inferSelect>,
  now: Date,
  rangeMs: number
) {
  const start = now.getTime() - rangeMs;
  return events
    .filter(event => event.occurredAt.getTime() >= start)
    .reduce(
      (sum, event) =>
        sum + asNumber(event.actualCostUsd ?? event.estimatedCostUsd),
      0
...
export async function saveUsageImport(input: {
  ...
  const format = input.filename.toLowerCase().endsWith(".json")
    ? "json"
    : "csv";
  ...
  if (!events.length) {
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: first
        ? `没有可导入的有效记录：第 ${first.row} 行${first.reason}`
        : "未检测到可导入的用量记录。",
    });
  }
  ...
  const duplicate = (
    await db
      .select()
      .from(usageImportBatches)
      .where(
        and(
          eq(usageImportBatches.workspaceId, input.workspaceId),
          eq(usageImportBatches.checksum, checksum)
        )
      )
      .limit(1)
  )[0];
  if (duplicate && duplicate.status !== "failed")
    throw new TRPCError({
      code: "CONFLICT",
      message: "该文件内容已导入过，请勿重复上传。",
    });

  let uploaded: { key: string; url: string } | undefined;
  try {
    uploaded = await storagePut(
      `quota-pilot/${input.workspaceId}/imports/${checksum}.${format}`,
      input.content,
      input.mimeType
    );
    ...
    const connections = await db
      .select()
      .from(providerConnections)
      .where(eq(providerConnections.workspaceId, input.workspaceId));
    ...
    const modelByProviderAndId = new Map(
      models.map(model => [`${model.provider}:${model.modelId}`, model])
    );
    const batchId = await db.transaction(async tx => {
      let createdBatchId =
        duplicate?.status === "failed" ? duplicate.id : undefined;
      if (createdBatchId) {
        await tx
          .update(usageImportBatches)
          .set({
      ...
      await tx.insert(usageEvents).values(
        events.map((event, index) => {
          ...
          const model = modelByProviderAndId.get(
          ...
          return {
            ...
            externalRef:
              event.externalRef || `batch:${createdBatchId}:row:${index}`,
          };
        })
      );
      await tx
    ...
    await refreshWorkspaceBudgets(input.workspaceId).catch(error =>
      console.error(
        "[QuotaPilot] import committed but budget refresh failed",
        error
      )
    );
    return {
    ...
    const failure = {
      ...
      errorSummary:
        error instanceof Error ? error.message.slice(0, 2000) : "导入事务失败",
      updatedAt: new Date(),
    ...
    if (duplicate?.status === "failed") {
      await db
        .update(usageImportBatches)
        .set(failure)
        .where(eq(usageImportBatches.id, duplicate.id))
        .catch(() => undefined);
    } else {
      await db
        .insert(usageImportBatches)
        .values(failure)
        .catch(() => undefined);
    }
    throw error;
...
async function getIdempotentTaskCreateResult(
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(researchTasks.idempotencyKey, input.idempotencyKey)
        )
      )
      .limit(1)
  )[0];
  ...
  const reserved =
    reservation?.reservationKind === "hard" &&
    reservation.status === "RESERVED";
  const softReserved =
    reservation?.reservationKind === "soft" &&
    reservation.status === "RESERVED";
  return {
    taskId: task.id,
    reserved,
    ...
    state:
      task.status === "reserved"
        ...
        : task.status === "paused"
          ? ("PAUSED" as const)
          : ("QUEUED" as const),
    admission: task.admissionDecision as
      | "ADMIT"
      | "RESERVE"
      | "MIGRATE"
      | "QUEUE"
...
export async function reserveTaskBudget(input: {
  ...
  if (!taskBudgetAdmission.admitted) {
    const code =
      input.estimatedCostUsd > input.taskBudgetUsd
        ? "PRECONDITION_FAILED"
        : "BAD_REQUEST";
  ...
  const [models, connections, budgets, runningAttempts] = await Promise.all([
    ...
    db
      ...
      .where(
        and(
          eq(providerBudgets.workspaceId, input.workspaceId),
  ...
  for (const attempt of runningAttempts) {
    ...
    if (modelId)
      activeConcurrencyByModel.set(
        modelId,
        (activeConcurrencyByModel.get(modelId) ?? 0) + 1
      );
  }
  const budgetByConnectionId = new Map(
    budgets.map(budget => [budget.providerConnectionId, budget])
  );
  const providerContexts = connections.map(connection => {
    ...
    return {
      ...
      availableUsd: budget
        ? asNumber(budget.limitUsd) -
          asNumber(budget.consumedUsd) -
          asNumber(budget.reservedUsd)
        : undefined,
      dynamicReserveUsd: budget
        ? asNumber(budget.dynamicReserveUsd)
        : undefined,
  ...
  const routingModels = models.map(model => ({
    ...
    maxConcurrency: Math.max(
      0,
      model.maxConcurrency - (activeConcurrencyByModel.get(model.modelId) ?? 0)
    ),
  ...
  const selectedModel = routingModels.find(
    model => model.modelId === recommendedModelId
  );
  const selectedRegistryModel = models.find(
    model =>
      model.modelId === recommendedModelId &&
      model.provider === (routing.selectedProvider ?? selectedModel?.provider)
  );
  const attemptProvider =
    routing.selectedProvider ?? selectedModel?.provider ?? "opencode_go";
  const budget =
    budgets.find(
      candidate =>
        candidate.providerConnectionId ===
        connections.find(connection => connection.provider === attemptProvider)
          ?.id
    ) ?? budgets[0];
  if (!budget)
    throw new TRPCError({
      code: "PRECONDITION_FAILED",
      message: "缺少可用于任务预留的五小时共享预算。",
    });
  const available =
    asNumber(budget.limitUsd) -
    asNumber(budget.consumedUsd) -
  ...
  const admission =
    routing.blockedByCapability || routing.blockedByBudget
      ? ("HOLD" as const)
  ...
  try {
    result = await db.transaction(async tx => {
      ...
      if (reservationKind === "hard" && admission !== "HOLD") {
        hardReservationCommitted = await claimInitialHardReservation(
          tx,
          budget.id,
          input.estimatedCostUsd
        );
      ...
      const decisionDetails =
        routing.blockedByCapability || routing.blockedByBudget
          ? {
              reason: routing.blockedByBudget
                ? "没有候选 provider 具备覆盖任务的共享可用额度；不能静默绕过预算。"
                : "没有模型满足任务的能力硬约束；不能为保持队列而静默降级。",
              action: "manual_handoff" as const,
              human: true,
            }
          : describeRouteDecision(effectiveAdmission);
      const selectedCandidate = candidates.find(
        candidate => candidate.modelId === recommendedModelId
      );
      ...
      const taskStatus = hardReservationCommitted
        ? "reserved"
        : effectiveAdmission === "HOLD" || effectiveAdmission === "MIGRATE"
          ? "paused"
          : "queued";
      await tx
      ...
      return {
        ...
        reservationKind,
        state: hardReservationCommitted
          ? ("RESERVED" as const)
          : effectiveAdmission === "HOLD"
            ? ("PAUSED" as const)
            : ("QUEUED" as const),
    ...
    if (isDuplicateKeyError(error)) {
      const concurrentResult = await getIdempotentTaskCreateResult(db, input);
  ...
  await refreshWorkspaceBudgets(input.workspaceId).catch(error =>
    console.error(
      "[QuotaPilot] reservation committed but budget refresh failed",
      error
    )
  );
  return result;
...
export async function evaluateRouteLabPolicy(input: {
    ...
    models: models.map(model => ({
      ...
      maxConcurrency: Math.max(
        ...
        model.maxConcurrency -
          (activeConcurrencyByModel.get(model.modelId) ?? 0)
      ),
      activeConcurrency: activeConcurrencyByModel.get(model.modelId) ?? 0,
      maxContextTokens: model.maxContextTokens,
      capability: model.capability,
  ...
  const selectedConnection = connections.find(
    connection => connection.provider === routing.selectedProvider
  );
  const selectedBudget = selectedConnection
    ? budgetByConnectionId.get(selectedConnection.id)
    : budgets[0];
  const budgetState = selectedBudget?.state ?? "GREEN";
  const availableUsd = selectedBudget
    ? asNumber(selectedBudget.limitUsd) -
      asNumber(selectedBudget.consumedUsd) -
      asNumber(selectedBudget.reservedUsd)
    : 0;
  const baseDecision =
    routing.blockedByCapability || routing.blockedByBudget
      ? ("HOLD" as const)
      : getAdmissionDecision({
          ...
          estimatedCostUsd: input.estimatedCostUsd,
          availableUsd,
          dynamicReserveUsd: asNumber(selectedBudget?.dynamicReserveUsd),
          budgetState,
        });
  ...
  return {
    evaluationId: Number(inserted[0].insertId),
...
export async function claimTaskForLocalExecution(input: {
  workspaceId: number;
  taskId: number;
}) {
  const db = await getDb();
  ...
  const result = await db.transaction(async tx => {
    ...
    if (!attempt)
      throw new TRPCError({
        code: "CONFLICT",
        message: "任务没有可领取的 queued attempt。",
      });
    if (attempt.retryNotBefore && attempt.retryNotBefore > new Date()) {
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: `该 attempt 正在退避，最早可于 ${attempt.retryNotBefore.toISOString()} 重新领取。`,
      });
    }
    const reservation = (
      await tx
        .select()
        .from(budgetReservations)
        .where(eq(budgetReservations.taskId, task.id))
        .limit(1)
    )[0];
    const connection = (
      await tx
    ...
    if (
      ...
      connection.circuitOpenUntil > new Date()
    ) {
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: `Provider 因 ${connection.circuitReason ?? "未知故障"} 正处于短路冷却期，最早可于 ${connection.circuitOpenUntil.toISOString()} 后重新领取。`,
      });
    }
    ...
    if (!budget)
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: "任务模型缺少可执行的五小时共享预算。",
      });
    const remainingTaskBudget =
      task.remainingBudgetUsd === null || task.remainingBudgetUsd === undefined
    ...
    if (asNumber(task.estimatedCostUsd) > remainingTaskBudget) {
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: "执行前任务成本上限校验失败；预计成本超过任务剩余预算。",
      });
    }
    ...
    const concurrencyModel = attempt.modelRegistryId
      ? (
          await tx
            ...
            .where(
              and(
                eq(modelRegistry.id, attempt.modelRegistryId),
                eq(modelRegistry.isActive, true)
              )
            )
            .limit(1)
              ...
              .where(
                and(
                  eq(
                    ...
                    attempt.requestedModelId ?? task.requestedModelId ?? ""
                  ),
                  eq(
                    modelRegistry.provider,
                    attempt.provider as "opencode_go" | "openai_api" | "local"
                  ),
                  eq(modelRegistry.isActive, true)
                )
              )
    ...
    if (attempt.modelRegistryId === null && !concurrencyModel) {
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: "旧 attempt 无法匹配活跃模型版本；拒绝绕过模型并发预算领取。",
      });
    }
    ...
    const releaseConcurrency = async () => {
      if (modelConcurrencyClaimed && concurrencyModel) {
        await tx
          .update(modelConcurrencyBudgets)
          .set({
            runningExecutions: sql`GREATEST(${modelConcurrencyBudgets.runningExecutions} - 1, 0)`,
            updatedAt: new Date(),
          })
          .where(
            and(
      ...
      if (providerConcurrencyClaimed && connection) {
        await tx
          ...
          .set({
            runningExecutions: sql`GREATEST(${providerConnections.runningExecutions} - 1, 0)`,
            updatedAt: new Date(),
          })
    ...
    if (concurrencyModel && connection) {
      const providerClaim = await tx
        .update(providerConnections)
        .set({
          runningExecutions: sql`${providerConnections.runningExecutions} + 1`,
          updatedAt: new Date(),
        })
        .where(
          and(
      ...
      if (providerClaim[0].affectedRows !== 1)
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "Provider 并发槽位已满；任务保持排队。",
        });
      providerConcurrencyClaimed = true;
      await tx
        .insert(modelConcurrencyBudgets)
        .values({
          workspaceId: input.workspaceId,
      ...
      const modelClaim = await tx
        .update(modelConcurrencyBudgets)
        .set({
          runningExecutions: sql`${modelConcurrencyBudgets.runningExecutions} + 1`,
          updatedAt: new Date(),
        })
        .where(
          and(
      ...
      if (modelClaim[0].affectedRows !== 1) {
        ...
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "模型并发槽位已满；任务保持排队。",
        });
      }
      modelConcurrencyClaimed = true;
    }

    let claimKind: "existing_hard" | "soft_upgraded" | "p3_hard" =
      "existing_hard";
    if (
      !reservation ||
      reservation.reservationKind === "soft" ||
      ...
      if (conditionalReserve[0].affectedRows !== 1) {
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "执行前共享额度二次准入失败；任务保持排队且不会超卖预算。",
        });
      }
      if (reservation) {
        await tx
        ...
        claimKind =
          reservation.reservationKind === "soft" ? "soft_upgraded" : "p3_hard";
      } else {
        await tx.insert(budgetReservations).values({
          workspaceId: input.workspaceId,
          providerBudgetId: budget.id,
    ...
    const attemptClaim = await tx
      .update(taskAttempts)
      .set({
        status: "running",
        modelRegistryId: concurrencyModel?.id ?? attempt.modelRegistryId,
        concurrencyClaimed: modelConcurrencyClaimed,
        startedAt: new Date(),
      })
      .where(
        and(eq(taskAttempts.id, attempt.id), eq(taskAttempts.status, "queued"))
      );
    if (attemptClaim[0].affectedRows !== 1) {
      await releaseConcurrency();
      throw new TRPCError({
        code: "CONFLICT",
        message: "attempt 已被其他执行者领取。",
      });
    }
    await tx.insert(taskEvents).values({
      workspaceId: input.workspaceId,
      ...
      payload: {
        ...
        claimKind,
        modelRegistryId: concurrencyModel?.id ?? attempt.modelRegistryId,
        executionPlan: attempt.executionPlan ?? null,
      },
    });
    return {
      taskId: task.id,
  ...
  const settled = await db.transaction(async tx => {
      ...
      await tx
        ...
        .where(
          and(
            ...
            eq(taskAttempts.taskId, input.taskId),
            eq(taskAttempts.workspaceId, input.workspaceId)
          )
        )
        .limit(1)
    ...
    if (
      ...
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message:
          "模型切换后的 attempt 不能写入 official 结果；请标记为 fallback 或 recovery。",
      });
    }
    ...
    const remainingTaskBudget =
      task.remainingBudgetUsd === null || task.remainingBudgetUsd === undefined
        ...
        : asNumber(task.remainingBudgetUsd);
    const nextActualCostUsd =
      asNumber(task.actualCostUsd) + input.actualCostUsd;
    if (
      input.actualCostUsd > remainingTaskBudget ||
      nextActualCostUsd > cumulativeCostCapUsd
    ) {
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: "实际成本超过任务累计成本上限或剩余预算，系统拒绝写入结算。",
      });
    }
    const finalResultClass =
      input.fallback || input.actualModelId !== task.requestedModelId
        ...
        : input.resultClass;
    const failureReason =
      input.status === "failed" ? (input.failureReason ?? "UNKNOWN") : null;
    const failurePolicy = failureReason
      ? getFailurePolicy(failureReason, task.priority)
      : null;
    const failureExecutionPlan = failureReason
      ? getFailureExecutionPlan(failureReason)
      : null;
        ...
        : input.status === "cancelled"
          ...
          : canScheduleRetry
            ...
            : failurePolicy?.recommendedAction === "hold" ||
                failurePolicy?.recommendedAction === "migrate" ||
                failurePolicy?.recommendedAction === "manual_handoff"
              ? "paused"
              : "failed";
    const completedAt = canScheduleRetry ? null : new Date();
    const didFallback =
      input.fallback || input.actualModelId !== task.requestedModelId;
    const fallbackReason = didFallback
      ? (input.fallbackReason ?? "manual")
      : null;
    const tokenSnapshot = {
    ...
    await tx
      ...
      .set({
        ...
        remainingBudgetUsd: Math.max(
          0,
          cumulativeCostCapUsd - nextActualCostUsd
        ).toFixed(6),
        resultClass: finalResultClass,
        status: finalTaskStatus,
        queuedAt: canScheduleRetry ? new Date() : task.queuedAt,
        completedAt,
        updatedAt: new Date(),
      })
    ...
    if (canScheduleRetry) {
      ...
      await tx.insert(taskEvents).values({
        workspaceId: input.workspaceId,
        taskId: task.id,
        attemptId: retryAttempt?.[0]?.insertId
          ? Number(retryAttempt[0].insertId)
          : null,
        kind: "retry_queued",
        payload: {
          attemptNumber: Number(attemptNumber) + 1,
    ...
    if (
      failurePolicy &&
      !canScheduleRetry &&
      failurePolicy.recommendedAction !== "queue"
    ) {
    ...
    const reservationStatus =
      input.status === "completed" ? "CONSUMED" : "RELEASED";
    await tx
      .update(budgetReservations)
      ...
      await tx
        ...
        .where(
          and(
            ...
            eq(
              providerConnections.provider,
              (attempt.provider ?? "opencode_go") as
                | "opencode_go"
                | "openai_api"
                | "chatgpt_plus_manual"
                | "local"
        ...
        await tx
          ...
          .where(
            and(
              eq(modelConcurrencyBudgets.workspaceId, input.workspaceId),
              eq(
                modelConcurrencyBudgets.modelRegistryId,
                attempt.modelRegistryId
              )
            )
          );
      }
    ...
    if (
      failurePolicy &&
      (failurePolicy.circuitScope === "provider" ||
        failurePolicy.circuitScope === "provider_window") &&
      connection
    ) {
      const circuitOpenUntil =
        retryNotBefore ?? new Date(Date.now() + 5 * 60 * 1_000);
      await tx
        .update(providerConnections)
        .set({
    ...
    await tx.insert(taskEvents).values({
      ...
      payload: {
        ...
        retryScheduledAt: retryNotBefore?.toISOString() ?? null,
      },
    });
    return {
      taskStatus: finalTaskStatus,
      ...
      failurePolicy,
      failureExecutionPlan,
      retryScheduledAt: retryNotBefore,
    };
  });
  await refreshWorkspaceBudgets(input.workspaceId);
  return settled;
...
export async function queueTaskRetry(input: {
  workspaceId: number;
  taskId: number;
}) {
  const db = await getDb();
  ...
  return db.transaction(async tx => {
    ...
    if (!task)
      throw new TRPCError({
        code: "CONFLICT",
        message: "任务当前不可重试；请先完成或取消正在运行的 attempt。",
      });
    ...
    const retryAdmission = getTaskRetryAdmission({
      attemptCount: attempts.length,
      maxAttempts: task.maxAttempts,
    });
    if (!retryAdmission.admitted)
      throw new TRPCError({
        code: "PRECONDITION_FAILED",
        message: retryAdmission.reason,
      });
    const previousAttempt = attempts.reduce(
      (latest, attempt) =>
...
export async function listWorkspaceDashboard(workspaceId: number) {
  ...
  const snapshots = await db
    .select()
    .from(quotaSnapshots)
    .where(eq(quotaSnapshots.workspaceId, workspaceId))
  ...
  const attempts = await db
    .select()
    .from(taskAttempts)
    .where(eq(taskAttempts.workspaceId, workspaceId))
  ...
  const decisions = await db
    .select()
    .from(routeDecisions)
    .where(eq(routeDecisions.workspaceId, workspaceId))
